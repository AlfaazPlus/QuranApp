...
class RecitationController private constructor(private val appContext: Context) {
    ...
    private fun onControllerConnected() {
        ...
        sendCommand(RecitationService.Companion.ACTION_GET_STATE, Bundle.EMPTY) { result ->
    ...
    fun play(command: PlaybackCommand) {
        ensureConnectedAndSend(RecitationService.Companion.ACTION_PLAY, command.toBundle())
    }
    ...
    fun play(chapter: Int, verse: Int) {
        ensureConnectedAndSend(RecitationService.Companion.ACTION_RECITE_VERSE, Bundle().apply {
            putInt(RecitationService.Companion.EXTRA_CHAPTER, chapter)
            putInt(RecitationService.Companion.EXTRA_VERSE, verse)
        })
    }
    ...
    fun stop() {
        sendCommand(RecitationService.Companion.ACTION_STOP, Bundle.EMPTY)
    }

    fun seekTo(positionMs: Long) {
        sendCommand(RecitationService.Companion.ACTION_SEEK, Bundle().apply {
            putLong(RecitationService.Companion.EXTRA_SEEK_AMOUNT, positionMs)
        })
    }

    fun seekLeft() {
        sendCommand(RecitationService.Companion.ACTION_SEEK, Bundle().apply {
            putLong(RecitationService.Companion.EXTRA_SEEK_AMOUNT, RecitationService.Companion.ACTION_SEEK_LEFT)
        })
    }

    fun seekRight() {
        sendCommand(RecitationService.Companion.ACTION_SEEK, Bundle().apply {
            putLong(RecitationService.Companion.EXTRA_SEEK_AMOUNT, RecitationService.Companion.ACTION_SEEK_RIGHT)
        })
    }

    fun previousVerse() {
        ensureConnectedAndSend(RecitationService.Companion.ACTION_PREVIOUS_VERSE, Bundle.EMPTY)
    }

    fun nextVerse() {
        ensureConnectedAndSend(RecitationService.Companion.ACTION_NEXT_VERSE, Bundle.EMPTY)
    }

    fun cancelLoading() {
        sendCommand(RecitationService.Companion.ACTION_CANCEL_LOADING, Bundle.EMPTY)
    }

    // ==================== Settings Commands ====================

    fun setSpeed(speed: Float) {
        ensureConnectedAndSend(RecitationService.Companion.ACTION_SET_PLAYBACK_SPEED, Bundle().apply {
            putFloat(RecitationService.Companion.EXTRA_SPEED, speed)
        })
    }

    fun setRepeat(repeat: Boolean) {
        ensureConnectedAndSend(RecitationService.Companion.ACTION_SET_REPEAT, Bundle().apply {
            putBoolean(RecitationService.Companion.EXTRA_REPEAT, repeat)
        })
    }

    fun setContinue(continuePlay: Boolean) {
        ensureConnectedAndSend(RecitationService.Companion.ACTION_SET_CONTINUE_PLAYING, Bundle().apply {
            putBoolean(RecitationService.Companion.EXTRA_CONTINUE, continuePlay)
        })
    }

    fun setVerseSync(sync: Boolean, fromUser: Boolean = false) {
        ensureConnectedAndSend(RecitationService.Companion.ACTION_SET_VERSE_SYNC, Bundle().apply {
            putBoolean(RecitationService.Companion.EXTRA_SYNC, sync)
            putBoolean(RecitationService.Companion.EXTRA_FROM_USER, fromUser)
        })
    }

    fun setAudioOption(option: Int) {
        ensureConnectedAndSend(RecitationService.Companion.ACTION_SET_AUDIO_OPTION, Bundle().apply {
            putInt(RecitationService.Companion.EXTRA_AUDIO_OPTION, option)
        })
    }
    ...
    fun setChapter(chapterNo: Int, fromVerse: Int, toVerse: Int, currentVerse: Int = fromVerse) {
        ensureConnectedAndSend(RecitationService.Companion.ACTION_SET_CHAPTER, Bundle().apply {
            putInt(RecitationService.Companion.EXTRA_CHAPTER, chapterNo)
            putInt(RecitationService.Companion.EXTRA_FROM_VERSE, fromVerse)
            putInt(RecitationService.Companion.EXTRA_TO_VERSE, toVerse)
            putInt(RecitationService.Companion.EXTRA_VERSE, currentVerse)
        })
    }

    fun setJuz(juzNo: Int) {
        ensureConnectedAndSend(RecitationService.Companion.ACTION_SET_JUZ, Bundle().apply {
            putInt(RecitationService.Companion.EXTRA_JUZ, juzNo)
        })
    }
