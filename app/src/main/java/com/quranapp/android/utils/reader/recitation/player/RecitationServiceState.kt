...
data class PlaybackCommand(
    val chapter: Int,
    val verse: Int,
    val rangeStart: ChapterVersePair,
    val rangeEnd: ChapterVersePair,
    val audioOption: Int = RecitationUtils.AUDIO_OPTION_DEFAULT
) {
    fun toBundle(): Bundle = Bundle().apply {
        putInt("chapter", chapter)
        putInt("verse", verse)
        putInt("range_start_ch", rangeStart.chapterNo)
        putInt("range_start_vs", rangeStart.verseNo)
        putInt("range_end_ch", rangeEnd.chapterNo)
        putInt("range_end_vs", rangeEnd.verseNo)
        putInt("audio_option", audioOption)
    }
    ...
    companion object {
        fun fromBundle(bundle: Bundle): PlaybackCommand? {
            ...
            return PlaybackCommand(
                chapter = chapter,
                verse = verse,
                rangeStart = ChapterVersePair(
                    bundle.getInt("range_start_ch", chapter),
                    bundle.getInt("range_start_vs", 1)
                ),
                rangeEnd = ChapterVersePair(
                    bundle.getInt("range_end_ch", chapter),
                    bundle.getInt("range_end_vs", verse)
                ),
                audioOption = bundle.getInt("audio_option", RecitationUtils.AUDIO_OPTION_DEFAULT)
            )
        }
    }
}
        ...
        fun fromBundle(bundle: Bundle): RecitationServiceState {
            ...
            return RecitationServiceState(
                ...
                settings = PlaybackSettings(
                    ...
                    audioOption = bundle.getInt(KEY_AUDIO_OPTION, RecitationUtils.AUDIO_OPTION_DEFAULT)
                ),
                lastEvent = event,
