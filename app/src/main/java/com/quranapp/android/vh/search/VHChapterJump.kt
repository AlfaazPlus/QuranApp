...
class VHChapterJump(private val chapterCard: ChapterCard, applyMargins: Boolean) :
    VHSearchResultBase(chapterCard) {
    init {
    ...
    override fun bind(model: SearchResultModelBase, pos: Int) {
        if (model is ChapterJumpModel) {
            chapterCard.apply {
                ...
                setOnClickListener { ReaderFactory.startChapter(it.context, model.chapterNo) }
            }
        }
    }
}